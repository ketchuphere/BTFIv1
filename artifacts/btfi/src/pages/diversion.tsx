...
function downloadDiversionPlan(data: any, crowd: number, duration: number, closure: boolean) {
  const today = new Date().toLocaleDateString("en-IN", { day: "2-digit", month: "long", year: "numeric" });
  const time = new Date().toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" });

  const content = `# BTFI — Diversion Operational Plan
## ${data.affectedRoad} — ${today} ${time}

**Diversion Type:** ${data.diversionType ?? "MAJOR DIVERSION"}  
**Status:** ${data.status ?? "Ready for deployment"}  
**Signage Boards Required:** ${data.signageBoardsCount ?? 24}
...
| Route Condition | ETA |
|---|---|
| Normal (pre-event) | ${data.normalEta ?? "12 mins"} |
| Gridlock (no diversion) | ${data.congestedEta ?? "42 mins"} |
| Diverted (via bypass) | ${data.divertedEta ?? "18 mins"} |
| **Time Saved** | **${data.savingMinutes ?? 24} minutes** |
...
${data.primaryRoute ?? "Residency Road → Richmond Circle → Cubbon Road bypass"}

### Secondary Bypass Channel
${data.secondaryRoute ?? "Trinity Circle → Ulsoor Lake Road → Commercial Street"}

---

## Simulation Parameters

- Crowd Size: ${crowd.toLocaleString()} attendees
- Event Duration: ${duration} minutes
- Road Closure: ${closure ? "Yes — Full cordon active" : "No — Partial lanes open"}
...
`;
  ...
  a.href = url;
  a.download = `BTFI_Diversion_Plan_${new Date().toISOString().slice(0, 10)}.md`;
  a.click();
...
export default function Diversion() {
  ...
  const handleDivert = () => {
    createDiversion.mutate({ data: { affectedRoad: "MG Road" } }, {
      onSuccess: (d) => {
        setDivResult(d);
        downloadDiversionPlan(d, crowd, duration, closure);
      },
    });
  };
